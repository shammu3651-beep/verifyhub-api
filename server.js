...
app.post('/api/email/send', authenticateToken, async (req, res) => {
    ...
    if (!to || !subject) return res.status(400).json({ success: false, message: "Missing required parameters" });
    const result = await sendEmailViaService({ to, subject, html, text, type, otp });
    if (result.success) res.status(200).json({ success: true, message: "Email transmitted successfully.", details: result.data });
    else res.status(500).json({ success: false, message: "Email relay failed.", error: result.error });
});
...
app.get('/api/records', authenticateToken, async (req, res) => {
    try {
        // 🔥 Filter out soft-deleted records from main UI
        const records = await Record.find({ isDeleted: { $ne: true } }).sort({ createdAt: -1 });
...
app.put('/api/records/:id', authenticateToken, async (req, res) => {
    try {
        ...
        if (oldRecord) notificationEngine.notifyRecordUpdate(oldRecord, updatedRecord);
        res.status(200).json(sanitizeRecord(updatedRecord));
    } catch (error) { res.status(500).json({ message: "Server Error", error: error.message }); }
});

// 🔥 SMART DELETE (Soft Delete - moves to Recycle Bin)
app.delete('/api/records/:id', authenticateToken, async (req, res) => {
    try {
        ...
        res.status(200).json({ message: "Record moved to Recycle Bin safely", record: sanitizeRecord(deletedRecord) });
    } catch (error) { res.status(500).json({ message: "Server Error", error: error.message }); }
});
...
app.post('/api/records/restore/:id', authenticateToken, async (req, res) => {
    try {
        const restoredRecord = await Record.findOneAndUpdate(
            { id: req.params.id }, 
            { isDeleted: false, deletedAt: null }, 
            { new: true }
        );
        if (!restoredRecord) return res.status(404).json({ message: "Record not found in bin" });
        res.status(200).json(sanitizeRecord(restoredRecord));
    } catch (error) { res.status(500).json({ message: "Server Error", error: error.message }); }
});

app.delete('/api/records/permanent/:id', authenticateToken, async (req, res) => {
    try {
        const result = await Record.findOneAndDelete({ id: req.params.id });
        if (!result) return res.status(404).json({ message: "Record not found" });
        res.status(200).json({ message: "Permanently destroyed from Database" });
    } catch (error) { res.status(500).json({ message: "Server Error", error: error.message }); }
});

app.delete('/api/recycle-bin/empty', authenticateToken, async (req, res) => {
    try {
        await Record.deleteMany({ isDeleted: true });
        res.status(200).json({ message: "Recycle Bin completely emptied." });
    } catch (error) { res.status(500).json({ message: "Server Error", error: error.message }); }
});
...
app.get('/api/whatsapp/get-dp/:phone', authenticateToken, async (req, res) => {
    try {
        ...
        if (url) res.status(200).json({ success: true, url: url });
...
app.post('/api/whatsapp/send/:phone', authenticateToken, async (req, res) => {
    try {
        const phone = req.params.phone;
        const { text } = req.body;
        if (!phone || !text) return res.status(400).json({ success: false, message: "Invalid payload" });
        const isSent = await sendAutoWaMessage(phone, text);
        if (isSent) res.status(200).json({ success: true, message: "Sent successfully via Cloud WA" });
...
app.post('/api/whatsapp/send-media/:phone', authenticateToken, async (req, res) => {
    try {
        const phone = req.params.phone;
        const { type } = req.body; 
        if (!phone || !type) return res.status(400).json({ success: false, message: "Invalid payload" });
...
app.get('/', (req, res) => { res.send("<h1>VerifyHub API</h1><p>Services Chalu Hai 🚀</p>"); });

const PORT = process.env.PORT || 5000;
app.listen(PORT, () => { console.log(`🚀 VerifyHub Backend Running on Port: ${PORT}`); });
